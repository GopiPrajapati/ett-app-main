...
import {getSearchedHubs} from '../../../../../redux/actions';
import {Routes} from '../../../../Routes';
import ListEmptyComponent from '../ListEmptyComponent';

const HubsTabScreen = ({searchTerm = ''}) => {
  ...
  const fetchResults = useCallback(
    query => {
      dispatch(
        getSearchedHubs(query, {
          SuccessCallback: response => {
            setHubs(response?.data?.shoppingHubs || []);
            setLoading(false);
          },
          FailureCallback: () => {
            setHubs([]);
