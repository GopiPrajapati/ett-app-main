...
import {getHubs, toggleLike} from '../../../redux/actions';
...
const HubsScreen = () => {
  ...
  const fetchHubs = useCallback(
    (isReset = false) => {
      ...
      setLoading(true);
      dispatch(
        getHubs(pageRef.current, 9, {
          SuccessCallback: response => {
            const {pagination, shoppingHubs} = response?.data;
            totalPagesRef.current = pagination?.totalPages;
            setData(prev =>
              isReset ? shoppingHubs : [...data, ...shoppingHubs],
            );
            pageRef.current += 1;
            setLoading(false);
          },
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <ScrollView
        ...
        <GreetingContainer
          illustrationVideo={
            global?.isDarkMode
              ? videos.ic_hubs_video_dark
              : videos.ic_hubs_video
          }
          bannerHeight={normalize(225)}
          ...
          <View style={{paddingHorizontal: sizes.paddingHorizontal}}>
            <TouchableOpacity>
              <Input
                ...
                placeholder={strings.search_hubs}
                editable={false}
                onPress={() => {
                  navigate(Routes.SEARCH_SCREEN, {key: Tabs.Hubs});
                  hapticFeedback();
                }}
